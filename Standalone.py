...
def main():
    ...
    print('Epoch of Local Training: ', epoch_num)
    ...
    for i in range(client_number):
        ...
        if i%10 ==9:
            print('The Average (Accuracy, Precision, Recall, F_measure) of the former ', i+1, ' users: ', \
                  sum(Accuracy) / (i+1), sum(Precision) / (i+1), sum(Recall) / (i+1), sum(F_measure) / (i+1))
